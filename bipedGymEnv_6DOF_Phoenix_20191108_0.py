...
Author: Phoenix Fan
Date: 08-11-2019
Specification:  Increase the velocity limit of joints.
                Decrease the total amount of penalty of joint efficiency.
                Decrease the weight of joint efficiency of hip joints.
                Increase the weight of joint efficiency of ankle joints.
                Recover death standard.
                Log NAN error, observations, actions, rewards into CSV.
                Change the log format to CSV.
                Add a pardon area on z_reward which means there is no penalty when the enter is in the area.
"""
...
class BipedRobot(gym.Env):
    ...
    def __init__(self, isGUI=True, useFixedBase=False, demonstration=False, reset_status=False,
        ...
        super()
        self.logging_float = []  # the information to be recorded in csv file consisting of float figures
    ...
    def load_bot(self):
        loc = '/home/antikythera1/workplace/Exoskeleton/biped_6DOF_20191108.urdf'
        self.center_datum[:, :] = 0.
    ...
    def get_limitation_space(self):
        ...
        # logging
        # write the limitation of observation and action
        # write the headline of csv logging file
        action = ['rHip', 'rKnee', 'rAnkle', 'lHip', 'lKnee', 'lAnkle']
        base = ['bvel'] * 3 + ['bacc'] * 3 + ['bori'] * 4 + ['boriv'] * 4
        tmp_link = [(['lvel_{}'.format(i)] * 3 + ['lacc_{}'.format(i)] * 3 + ['lori_{}'.format(i)] * 4
                    + ['loriv_'.format(i)] * 4 + ['lrelp_'.format(i)] * 3) for i in range(6)]
        link = []
        for i in tmp_link:
            link += tmp_link
        joint = ['vel', 'acc', 'relp']
        reward = ['x', 'y', 'z', 'surv', 'j0', 'j1', 'j2', 'j3', 'j4', 'j5']
        with open(self.log_name, 'a') as csv_log:
            csv_writer = csv.writer(csv_log)
            csv_writer.writerow(['observation_space'])
            csv_writer.writerow(self.observation_space.high)
            csv_writer.writerow(self.observation_space.low)
            csv_writer.writerow(['action_space'])
            csv_writer.writerow(self.action_space.high)
            csv_writer.writerow(self.action_space.low)
            csv_writer.writerow(action + base + link + joint + reward)
        return
    ...
    def get_observations(self):
        ...
        if link_nan_flag or joint_nan_flag:
            if self.log_flag is True:
                with open(self.log_name, mode='a') as csv_log:
                    csv_writer = csv.writer(csv_log)
                    csv_writer.writerow(['Not a number error'])
    ...
    def reward(self, dead, observations):
        ...
        z_reward = 0 if abs(self.torso_position[1, 2] - self.avgTorsoCenterHeight) < 0.1 else (-abs(self.torso_position[1, 2] - self.avgTorsoCenterHeight))
        ...
        hip_efficiency_coefficient = -0.01
        knee_efficiency_coefficient = -0.05
        ankle_efficiency_coefficient = -0.2

        x_reward *= x_coefficient
        y_reward *= y_coefficient
        z_reward *= z_coefficient
        survival *= survival_coefficient
        joint_efficiency[0] *= hip_efficiency_coefficient
        joint_efficiency[1] *= knee_efficiency_coefficient
        joint_efficiency[2] *= ankle_efficiency_coefficient
        joint_efficiency[3] *= hip_efficiency_coefficient
        joint_efficiency[4] *= knee_efficiency_coefficient
        joint_efficiency[5] *= ankle_efficiency_coefficient

        self.avg_x_reward += x_reward
        self.avg_y_reward += y_reward
        self.avg_z_reward += z_reward
        self.avg_survival_reward += survival
        # self.avg_joint_efficiency = [self.avg_joint_efficiency[i] + tmp_joint_efficiency[i] for i in range(self.number_joint)]
        self.avg_joint_efficiency = np.add(self.avg_joint_efficiency, joint_efficiency)

        # logging: rewards
        self.logging_float += ([x_reward, y_reward, z_reward, survival_coefficient] + joint_efficiency)

        # logging the action, observations, rewards
        if self.log_flag is True:
            with open(self.log_name, mode='a') as csv_log:
                csv_writer = csv.writer(csv_log)
                csv_writer.writerow(self.logging_float)

        return x_reward + y_reward + z_reward + survival + sum(joint_efficiency)
    ...
    def step(self, target_velocity):
        ...
        # logging: action
        self.logging_float = target_velocity.tolist()
        ...
        # logging: observation
        self.logging_float += observations.tolist()
