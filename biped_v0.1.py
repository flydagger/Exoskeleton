...
Author: Phoenix Fan
Date: 18-09-2019
Target: 1. Exoskeleton 12 DOF Run with a speed.
        2. Two soles are not allowed to leave the gound at the same time.
"""
...
class BipedRobot(gym.Env):
    ...
    def __init__(self, isGUI=True, useFixedBase=False, demonstration=False, reset_status=True,
        ...
        self.loadBot()
        self.jointNum = self.p.getNumJoints(self.botId) - 1  # number of useful joints
    ...
    def gatherJointsInfo(self):
        ...
        velocity_spaces = np.zeros([self.jointNum, 2, ], dtype=float)  # speed limit of each joint
        joints_limit_low = np.zeros([self.jointNum * 2, ],
                                    dtype=float)  # [0, self.jointNum-1] joint position limit, [self.jointNum, self.jointNum*2-1] joint velocity limit
        joints_limit_high = np.zeros([self.jointNum * 2, ],
                                     dtype=float)  # [0, self.jointNum-1] joint position limit, [self.jointNum, self.jointNum*2-1] joint velocity limit
        ...
        for jointIndex in range(1, jointAmount):
            ...
            joints_limit_low[jointIndex - 1 + self.jointNum] = -JInfo[11]  # lower velocity limit
            joints_limit_high[jointIndex - 1 + self.jointNum] = JInfo[11]  # upper velocity limit
    ...
    def getObservations(self, botId=0, totalStep=0):
        ...
        # self.jointNum = Nj - 1
        jointStatus = np.zeros([self.jointNum * 2, ], dtype=float)
        ...
        for i in range(Nj):
            ...
            if i == 0:
                ...
                jointStatus[i - 1] = joint_state[0]  # joint position value [0, self.jointNum-1]
                jointStatus[i - 1 + self.jointNum] = joint_state[
                    1]  # joint velocity value [self.jointNum, self.jointNum*2-1]
                if i == (self.jointNum - 1) / 2:  # right foot
                ...
                elif i == self.jointNum - 1:  # left foot
        ...
        if result.__len__() == (self.jointNum * 2 + 12 + 6):
            return result
        else:
            return None
    ...
    def done(self, torso_position, left_foot_status, right_foot_status):
        """
        Target is running. Horizontal limit of x is [-1, 10]. Horizontal limit of y is [-1, 1].
        :param torso_position:
        :param left_foot_status:
        :param right_foot_status:
        :return:
        """
        # assume 240 steps nearly equals to one second
        # both soles do not touch the ground at the same time
        if torso_position[2] < self.deadLine \
                or self.time_limit > 240 * 10 \
                or torso_position[1] > 1 \
                or torso_position[1] < -1 \
                or torso_position[0] > 10 \
                or torso_position[0] < -1 \
                or (left_foot_status[0][2] > 0.15 and right_foot_status[0][2] > 0.15):
            return True
        else:
            return False
    ...
    def step(self, action, total_step=0):
        ...
        self.p.setJointMotorControlArray(bodyUniqueId=self.botId,
                                         jointIndices=[i for i in range(1, self.jointNum + 1)],
                                         controlMode=self.control_mode,
                                         forces=action)

        self.p.stepSimulation()
        torso_position = self.p.getLinkState(self.botId, 0)[0]  # Cartesian position of torso, (x, y, z)
        left_foot_status = self.p.getLinkState(self.botId, self.jointNum - 1)
        right_foot_status = self.p.getLinkState(self.botId, (self.jointNum - 1) / 2)
        done = self.done(torso_position, left_foot_status, right_foot_status)  # True means the robot is dead.
        observation = self.getObservations(self.botId, total_step)  # np array (42, )
        # newDistance = self.computeDistance(torso_position)
        # oldDistance = self.computeDistance(self.previous_position)
        ...
        # print("At step " + str(self.step_counter) + "  observation: " + str(observation))
        # print("At step " + str(self.step_counter) + "  reward: " + str(reward))
        # print("At step " + str(self.step_counter) + "  done: " + str(done))
